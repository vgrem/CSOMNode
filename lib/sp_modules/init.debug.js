...
function UpdateUrlWhenServerRedirects() {
    ...
    if (Flighting.VariantConfiguration.IsExpFeatureClientEnabled(708) && !(window["OffSwitch"] == null || OffSwitch.IsActive("FFE41112-6670-4E14-8F3D-EEED42BDF9AE")) && Boolean(pageContextInfo) && Boolean(pageContextInfo.serverRedirectedUrl)) {
        if (Boolean(window.history) && Boolean(window.history.replaceState)) {
            window.history.replaceState({
                reason: "SPServerRedirect"
            }, "SPServerRedirect", pageContextInfo.serverRedirectedUrl);
        }
        else {
            window.location.href = pageContextInfo.serverRedirectedUrl;
        }
    }
}
...
function SPThemeUtils_module_def() {
    ...
    function ClientSideThemingOptOut() {
        ...
        case 64:
            cstOptOut = !FlightingAvailable() || !Flighting.VariantConfiguration.IsExpFeatureClientEnabled(338);
            break;
        }
        return cstOptOut;
    }
    function ApplyCurrentTheme(hideContentWhileTheming) {
        ...
        if (FlightingAvailable() && ClientSideThemingOptOut()) {
